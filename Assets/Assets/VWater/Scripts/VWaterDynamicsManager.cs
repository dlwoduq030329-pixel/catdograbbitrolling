...
namespace Vertigo2 {
    public class VWaterDynamicsManager : MonoBehaviour
    {
        ...
        public void AddFlow(Vector3 position, float radius, float amplitude, int priority = 0)
        {
            ...
            // 1️⃣ 로컬에서 ripple 처리
            InternalAddFlow(position, radius, amplitude, priority);

            // 2️⃣ 다른 클라이언트에게 RPC 전송
            if (PhotonNetwork.IsConnected && pv != null)
            {
                pv.RPC("RPC_AddFlow", RpcTarget.Others, position, radius, amplitude, priority);
            }
        }
        ...
        void InternalAddFlow(Vector3 position, float radius, float amplitude, int priority)
        {
            // 기존 AddFlow 내부 로직
            // GPU Compute Shader에 전달
            Debug.Log($"Ripple added at {position}, radius={radius}, amp={amplitude}");
        }

        [PunRPC]
        void RPC_AddFlow(Vector3 position, float radius, float amplitude, int priority)
        {
            // 다른 클라이언트에서 AddFlow 처리
            InternalAddFlow(position, radius, amplitude, priority);
        }
    }
}