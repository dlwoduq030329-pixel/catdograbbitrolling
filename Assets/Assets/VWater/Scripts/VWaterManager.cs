...
namespace Vertigo2 
{
    public class VWaterManager : MonoBehaviour
    {
        ...
        public static void TriggerSplash(VWaterBase water, VWaterBody body)
        {
            ...
            bool isPlayer = body.gameObject.CompareTag("Player");
            pv.RPC("RPCSplash", RpcTarget.Others, spawnPos,isPlayer);
        }

        [PunRPC]
        public void RPCSplash(Vector3 pos, bool isPlayer)
        {
           
            GameObject splashPrefab = splashPrefab_Large_Heavy;
            GameObject splash = Instantiate(splashPrefab, pos, splashPrefab.transform.rotation);
            if (isPlayer)
            {
                splash.transform.localScale *= 30;
            }
        }
        ...
        public void Splash(VWaterBase water, VWaterBody body)
        {
            if (Mathf.Abs(body.velocity.y) > splashVelocity)
            {
                ...
                GameObject splash = Instantiate(splashPrefab, spawnPos, splashPrefab.transform.rotation);
