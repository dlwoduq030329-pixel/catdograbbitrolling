using System.Collections;
using UnityEngine;

/// <summary>
/// Player와 Enemy가 공유하는 Transform 위치 이동 연출이다.
/// 경로, 이동 가능 여부, MP와 행동 규칙은 판단하지 않는다.
/// </summary>
public static class BattleTransformMovement
{
    /// <summary>회전 없이 순간적으로 방향만 바꾸는 것을 막기 위한 초당 최대 회전각(도)이다.</summary>
    private const float RotationDegreesPerSecond = 720f;

    /// <summary>
    /// 지정 시간 동안 현재 위치에서 목표 위치로 이동하고 마지막 위치를 정확히 맞춘다.
    /// 이동하는 동안 진행 방향(XZ 평면 기준)을 바라보도록 함께 회전한다.
    /// </summary>
    public static IEnumerator MoveToPosition(
        Transform target,
        Vector3 targetPosition,
        float duration)
    {
        if (target == null)
        {
            yield break;
        }

        float safeDuration = Mathf.Max(0.01f, duration);
        Vector3 startPosition = target.position;

        Vector3 flatDirection = targetPosition - startPosition;
        flatDirection.y = 0f;
        Quaternion? faceDirection = flatDirection.sqrMagnitude > 0.0001f
            ? Quaternion.LookRotation(flatDirection.normalized, Vector3.up)
            : (Quaternion?)null;

        float elapsed = 0f;
        while (elapsed < safeDuration)
        {
            elapsed += Time.unscaledDeltaTime;
            target.position = Vector3.Lerp(
                startPosition,
                targetPosition,
                Mathf.Clamp01(elapsed / safeDuration));

            if (faceDirection.HasValue)
            {
                target.rotation = Quaternion.RotateTowards(
                    target.rotation,
                    faceDirection.Value,
                RotationDegreesPerSecond * Time.unscaledDeltaTime);
            }

            yield return null;
        }

        target.position = targetPosition;
        if (faceDirection.HasValue)
        {
            target.rotation = faceDirection.Value;
        }
    }

    /// <summary>
    /// 이동 없이 제자리에서 목표 위치 방향(XZ 평면 기준)을 즉시 바라보게 한다.
    /// 이동하지 않고 바로 공격할 때(상하좌우 인접 대상) 항상 정면만 보고 공격하는 문제를 막기 위해 사용한다.
    /// </summary>
    public static void FaceTowards(Transform target, Vector3 targetPosition)
    {
        if (target == null)
        {
            return;
        }

        Vector3 flatDirection = targetPosition - target.position;
        flatDirection.y = 0f;
        if (flatDirection.sqrMagnitude <= 0.0001f)
        {
            return;
        }

        target.rotation = Quaternion.LookRotation(flatDirection.normalized, Vector3.up);
    }
}
