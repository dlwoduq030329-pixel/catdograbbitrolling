...
public static class BattleCardMovementService
{
    ...
    /// Enemy 프리팹에 저장된 공용 상태 저장소에 기절을 적용한다.
    /// </summary>
    private static void ApplyEnemyCollisionStun(GameObject pushedEnemy)
    {
        BattleStatusEffects statusEffects = pushedEnemy.GetComponent<BattleStatusEffects>();
        if (statusEffects == null)
        {
            Debug.LogError($"[카드 밀치기] {pushedEnemy.name}에 BattleStatusEffects가 없습니다.", pushedEnemy);
            return;
        }
    ...
    /// Enemy 프리팹에 저장된 공용 상태 저장소에 기절과 이동 불가를 적용한다.
    /// </summary>
