...
public sealed class BattleCardActionController : MonoBehaviour
{
    ...
    public bool IsActionActive => IsSelectingTarget || IsAwaitingConfirmation || isResolvingDiceRoll;
    ...
    private BattleCardUseExecutor useExecutor;
    private bool isResolvingDiceRoll;

    // [3] 대상 사거리 계산·표시
    // 계산된 유효 타일을 저장하고, 일반 사거리와 효과 범위의 색상 표시·복구에 사용한다.
    private BattleCardTargetController targetController;
    ...
    public void Setup(
        ...
        BattleDiceSystem effectDiceSystem)
    {
        ...
        bool cardsCanBeUsedThisTurn)
    {
        ...
        if (selectedHandCard == null || selectedHandCard.CardData == null ||
            ...
            IsActionActive || !cardsCanBeUsedThisTurn)
        {
            return false;
        }
        ...
        int requiredMp = useExecutor.GetMpCost(selectedHandCard.ActionInfo, selectedHandCard.CardData);
        if (!useExecutor.CanPay(requiredMp))
        {
            Debug.Log($"카드 사용 불가: MP {requiredMp}이 필요합니다.", this);
            return false;
        }
            ...
            targetType == BattleCardTargetType.AllEnemies)
        {
            ...
            selectedTargetTile = targetController.FindTile(player.transform.position);
            ShowSelectedCardEffectRange();
            EnterCardUseConfirmationState();
        }
        else
        {
    ...
    public bool TrySelectTarget(GameObject chosenTarget, MapInfo chosenTargetTile)
    {
        ...
        if (!IsSelectingTarget || chosenTarget == null || chosenTargetTile == null ||
            !targetController.IsInTargetRange(chosenTargetTile))
        {
            return false;
        }
    ...
    /// <summary>단일 대상 카드는 기존 대상 사거리 표시를 유지하고, 범위 카드만 효과 범위로 바꿉니다.</summary>
    private void ShowSelectedCardEffectRange()
    {
        if (selectedCardInfo?.CardData == null ||
            selectedCardInfo.CardData.areaType == BattleCardAreaType.Single)
        {
            return;
        }

        RangeVisibilityChanged?.Invoke(
            targetController.ShowEffectRange(selectedCardInfo, selectedTargetTile));
    }
    ...
    public bool TryConfirmUse()
    {
        ...
        if (!useExecutor.TryPay(requiredMp, drawSystem, selectedCardInfo))
        {
            CancelAll();
            return false;
        }

        // 굴림 결과를 보고 취소하거나 다른 행동을 시작하지 못하도록 소비 직후 카드 행동과 전투 입력을 잠근다.
        GameObject usedCardTarget = selectedTarget;
        BattleActionRequest completedCardAction = selectedCardInfo.ActionInfo;
        BattleCardData usedCardData = selectedCardInfo.CardData;
        isResolvingDiceRoll = true;
        BattleGameManager.Instance?.LockBattleInputForCardRoll();

        // 주사위 컴포넌트가 누락된 이전 Scene에서도 이미 소비된 카드를 멈추지 않는다.
        // 이 경우 경고를 남기고 Normal 결과 한 번으로 같은 완료 경로를 실행한다.
        if (!useExecutor.TryStartRoll(
                usedCardData,
                result => CompleteConfirmedCardUse(
                    result,
                    effectContext,
                    preparedCardEffects,
                    completedCardAction,
        ...
        {
            Debug.LogWarning(
                "[Card Roll] 주사위 굴림을 시작하지 못해 Normal 결과로 카드 효과를 실행합니다.",
                this);
            CompleteConfirmedCardUse(
                CardDiceResult.Normal,
                effectContext,
                preparedCardEffects,
                completedCardAction,
                usedCardTarget,
    ...
    /// <summary>
    /// 한 번 확정된 굴림 결과를 카드 효과 전체와 반복 타격에 공통 적용한 뒤 입력 잠금을 해제한다.
    /// </summary>
    private void CompleteConfirmedCardUse(
        CardDiceResult rollResult,
        BattleCardEffectPipeline.Context effectContext,
        BattleCardEffectPipeline.PreparedUse preparedCardEffects,
        BattleActionRequest completedCardAction,
        GameObject usedCardTarget,
        int requiredMp)
    {
        try
        {
            // 주사위 연출 중 Player가 죽거나 전투가 종료됐다면 소비된 카드의 효과를 뒤늦게 적용하지 않는다.
            if (BattleGameManager.Instance == null)
            {
                Debug.LogError("[Card Roll] BattleGameManager가 없어 카드 효과 실행을 중단합니다.", this);
                ClearStateAndRange();
                return;
            }

            if (BattleGameManager.Instance.IsBattleStopped)
            {
                Debug.LogWarning("[Card Roll] 전투가 중지된 상태라 카드 효과 실행을 중단합니다.", this);
                ClearStateAndRange();
                return;
            }
            ...
            if (player == null)
            {
                Debug.LogError("[Card Roll] 카드 사용자 Player가 사라져 효과 실행을 중단합니다.", this);
                ClearStateAndRange();
                return;
            }

            effectContext.RollResult = rollResult;
            Debug.Log(
                $"[Card Roll] 눈 {rollResult.DiceNumber}, 판정 {rollResult.ResultGrade}, " +
                $"효과 배율 x{rollResult.Multiplier:0.##}",
                this);
        ...
        {
            isResolvingDiceRoll = false;
            BattleGameManager.Instance?.UnlockBattleInputAfterCardRoll();
        }
    }
    ...
    private bool IsSelectedCardTargetStillValid(bool cardUsesAutomaticEnemyTargeting)
    {
        ...
        return selectedTarget != null &&
               ...
               targetController.IsTargetValid(selectedTarget, selectedTargetTile);
    }
    ...
    private bool TryCalculateAffordableCardMpCost(out int requiredMp)
    {
        requiredMp = selectedCardInfo != null
            ? useExecutor.GetMpCost(selectedCardInfo.ActionInfo, selectedCardInfo.CardData)
            : 0;
        return useExecutor != null && useExecutor.CanPay(requiredMp);
    }
    ...
    private BattleCardEffectPipeline.Context CreateCardEffectExecutionContext()
    {
        return new BattleCardEffectPipeline.Context
        {
            ...
            FindNearestTileAtPosition = targetController.FindTile,
            ApplyStatusToTarget = ApplyStatusToUnit,
            PersistentAreaVisualizer = targetController.RangeView,
            PersistentAreaTileColor = targetController.EffectColor,
    ...
    private void EnterTargetSelectionState()
    {
        ...
        selectedTarget = null;
        selectedTargetTile = null;
        if (!targetController.ShowTargetRange(selectedCardInfo))
        {
            CancelAll();
            return;
        }
    ...
    private void EnterCardUseConfirmationState()
    {
        ...
        targetController.ShowPushPreview(selectedCardInfo, selectedTarget);
